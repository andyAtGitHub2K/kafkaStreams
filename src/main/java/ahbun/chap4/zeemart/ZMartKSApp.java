...
public class ZMartKSApp {
    ...
    private static Logger logger = LoggerFactory.getLogger(ZMartKSApp.class);
    ...
    private static Topology getTopology(final Properties properties, final Properties businessProperties) {
        ...
        KStream<String, Purchase>[] streams = ksMasked.branch(beerPredicate, bookPredicate);
        //STREAM_BRANCH.BEER.ordinal() properties.getProperty("beer_sink")

        streams[STREAM_BRANCH.BEER.ordinal()].to("beer-out",
                Produced.with(stringSerde, purchaseSerde));
        //STREAM_BRANCH.BOOK.ordinal() properties.getProperty("book_sink")
        streams[STREAM_BRANCH.BOOK.ordinal()].to("book-out",
                Produced.with(stringSerde, purchaseSerde));
